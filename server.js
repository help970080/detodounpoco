    ...
    try {
        ...
        app.get('/api/products', async (req, res) => {
            ...
            try {
                ...
                const productsWithFullUrl = products.map(product => ({
                    ...product,
                    images: product.images ? product.images.map(img => `https://detodounpoco.onrender.com${img}`) : []
                }));
        ...
        app.get('/api/products/:id', async (req, res) => {
            ...
            try {
                ...
                if (product) {
                    product.images = product.images ? product.images.map(img => `https://detodounpoco.onrender.com${img}`) : [];
                    res.json(product);
                } else {
                    res.status(404).json({ message: 'Producto no encontrado' });
                }
